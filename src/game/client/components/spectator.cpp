...
{
	int CurIndex = -1;
	const CNetObj_PlayerInfo **paPlayerInfos = GameClient()->m_RClient.GetSortedPlayersSpectatorArray(g_Config.m_RcSpectatorSortId);
...
void CSpectator::OnRender()
{
	...
	{
		if(!GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i) || GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_Team == TEAM_SPECTATORS)
			continue;
		...
		const CNetObj_PlayerInfo *pInfo = GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i);
		int DDTeam = GameClient()->m_Teams.Team(pInfo->m_ClientId);
		int NextDDTeam = 0;

		for(int j = i + 1; j < MAX_CLIENTS; j++)
		{
			const CNetObj_PlayerInfo *pInfo2 = GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, j);
		...
		if((Client()->State() == IClient::STATE_DEMOPLAYBACK && GameClient()->m_DemoSpecId == GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId) || (Client()->State() != IClient::STATE_DEMOPLAYBACK && GameClient()->m_Snap.m_SpecInfo.m_SpectatorId == GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId))
		{
		...
		{
			m_SelectedSpectatorId = GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId;
			PlayerSelected = true;
			if(MousePressed)
			{
				if(GameClient()->m_MultiViewActivated)
		...
		if(Client()->State() == IClient::STATE_DEMOPLAYBACK &&
			!GameClient()->m_Snap.m_aCharacters[GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId].m_Active)
		{
		...
		{
			char aClientId[16];
			GameClient()->FormatClientId(GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId, aClientId, EClientIdFormat::INDENT_AUTO);
		...
		TextRender()->TextEx(&NameCursor, GameClient()->m_aClients[GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId].m_aName);

		if(GameClient()->m_MultiViewActivated)
		{
			if(GameClient()->m_aMultiViewId[GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId])
			{
		...
		CTeeRenderInfo TeeInfo = GameClient()->m_aClients[GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId].m_RenderInfo;
		...
		if(GameClient()->m_aClients[GameClient()->m_RClient.GetSortedPlayersSpectator(g_Config.m_RcSpectatorSortId, i)->m_ClientId].m_Friend)
		{
