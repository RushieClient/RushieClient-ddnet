...
{
	const float RechargeBarWidth = 64.0f;
	const float RechargeBarHalfWidth = 32.0f;
	const float RechargeBarHeight = 16.0f;
	...
	const int CurTick = g_Config.m_ClPredict ? Client()->PredGameTick(g_Config.m_ClDummy) : GameClient()->m_GameWorld.GameTick();

	const int TicksLeft = RechargeTick - CurTick;
	if(TicksLeft <= 0)
		return;

	float RechargeProgress = std::clamp(TicksLeft, 0, RechargeTime) / (float)RechargeTime;
	if(RechargeProgress <= 0.0f)
	{
		return;
	}
	...
	RenderRechargeBarPos(Position.x, Position.y, RechargeBarWidth, RechargeBarHeight, RechargeProgress, Alpha);
}

void CRechargeBars::RenderRechargeBarPos(float x, const float y, const float Width, const float Height, float Progress, const float Alpha)
{
...
inline bool CRechargeBars::IsPlayerInfoAvailable(int ClientId) const
{
...
void CRechargeBars::OnRender()
{
	if(Client()->State() != IClient::STATE_ONLINE && Client()->State() != IClient::STATE_DEMOPLAYBACK)
		return;

	if(!g_Config.m_RcShowRechargeBars)
	{
		return;
	}
	...
	//
	// it is assumed that the tee with the Recharge bar fit into a 240x240 box centered on the tee
	...
	// render everyone else's Recharge bar, then our own
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		...
		RenderRechargeBar(ClientId);
	}
	if(LocalClientId != -1 && IsPlayerInfoAvailable(LocalClientId))
	{
		RenderRechargeBar(LocalClientId);
	}
}
